...
async function StripeGateway(req, res){
        ...
        console.log(data, "this is request body");
        //call the stripe api to process payment
        let stripe_response = true;

        if(stripe_response == false){
            let processed_ticket = await database.eventgo_schema().ProcessedTicket(data)
        ...
        let deleted = await database.eventgo_schema().Ticket(data).Delete();
